...
function obj:__tostring()
	if self.name=="default" then
		return string.format("%s(%d ,%d ,%d ,%d)",obj.__tag,self.x,self.y,self.width,self.height)
	elseif locationMode==1 then
		return string.format("%s \"%s\" \"%d,%d,%d,%d\"",obj.__tag,
			self.name,
			self.x,
			self.y,
			self.x+self.width,
...
function obj:__add(r)
	...
	return obj:__call(math.min(r1tl.x,r2tl.x),math.min(r1tl.y,r2tl.y),math.max(r1br.x,r2br.x)-math.min(r1tl.x,r2tl.x),math.max(r1br.y,r2br.y)-math.min(r1tl.y,r2tl.y))
end
--交集
function obj:__sub(r)
	...
	local r2tl,r2br=r:tl(),r:br()
	--矩形不相交时返回ZERO
	if not self:isintersect(r) then
		return obj.ZERO
	end
	return obj:__call(math.max(r1tl.x,r2tl.x),math.max(r1tl.y,r2tl.y),math.min(r1br.x,r2br.x)-math.max(r1tl.x,r2tl.x),math.min(r1br.y,r2br.y)-math.max(r1tl.y,r2tl.y))
end
--缩放
function obj:__mul(n)
	...
	local s=self:size()
	local as=s*n
	return obj:__call(math.round(c.x-as.width/2),math.round(c.y-as.height/2),as.width,as.height)
end
--属性叠加
function obj:__div(r)
	return obj:__call(self.x+r.x,self.y+r.y,self.width+r.width,self.height+r.height)
end
--相等
function obj:__eq(s)
	return self.width==s.width and self.height==s.height and self.x==s.x and self.y==s.y
end
function obj:__call(...)
	local o={
		x=0,
	...
	if #t==1 and type(t[1])=="string" then
		...
		o.x,o.y,o.width,o.height=math.round(x),math.round(y),math.round(math.max(0,w)),math.round(math.max(0,h))
		return o
	end
end
--元表设置结束
...
function funcValues.randomPoint(self)
	return Point(math.random(self.x,self.x+self.width),math.random(self.y,self.y+self.height))
end
...
--获取左上角
function obj.tl(self)
	return Point(self.x,self.y)
end
--获取右下角
function obj.br(self)
...
function obj.contains(self,p)
	if type(p)=="Point" then
		return self.x<=p.x and self.y<=p.y and self.x+self.width>=p.x and self.y+self.height>=p.y
	elseif type(p)=="Rect" then
		return self.x<=p.x and self.y<=p.y and self.x+self.width>=p.x+p.width and self.y+self.height>=p.y+p.height
	elseif type(p)=="Circle" then
		local c=p:center()
		return c.x-self.x>=p.r and c.y-self.y>=p.r and self.x+self.width-c.x>=p.r and self.y+self.height-c.y>=p.r
	end
end
--包含2个矩形的最小矩形
function obj.union(self,r)
	if type(r)=="Rect" then
		return self + r
	end
end
--取交集
function obj.intersect(self,r)
	if type(r)=="Rect" then
		return self - r
	end
end
--获取中心点
function obj.center(r)
	return Point(r.x+r.width/2,r.y+r.height/2)
end
--矩形是否相交(边与边重合视为相交)
function obj.isintersect(r1,r2)
	return r1.x<=r2.x+r2.width and r1.y<=r2.y+r2.height and r1.x+r1.width>=r2.x and r1.y+r1.height>=r2.y
end
--获取外接圆
...
function obj.setLocationMode(mode)
	locationMode=mode
end
...
function obj.toNaiveRect(self)
	if _G.type(naiveRect)~="userdata" then
		Zlog.fatal("无法获取全局拓展Rect类,转换失败,这可能是运行引擎版本错误造成的,请尝试更换至2.0或更新版本的引擎再运行,或者开启低版本兼容模式")
	end

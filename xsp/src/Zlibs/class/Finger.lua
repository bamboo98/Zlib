...
local function getFreeFinger()
    ...
    Zlog.error("所有手指都已经被占用,获取空闲手指ID失败,将返回默认ID(0),在某些情况下将会出现触摸操作错误")
    return 0
end
...
local function getCurveWay(pos,poscount,speed,way)
    ...
    for i=0,poscount-2 do
        ...
        local extraindex=2*i
        controlPoints[2]={extrapoints[1][extraindex+1],extrapoints[2][extraindex+1]}
        local extranexti=(extraindex+2)%(2*poscount)
        controlPoints[3]={extrapoints[1][extranexti],extrapoints[2][extranexti]}
        local nexti=(i+1)%poscount
...
function obj:__tostring()
	return string.format("%s id = %d , %s ",obj.__tag,self.id,self.x==-1 and "未按下" or ("按下: "..self.x..","..self.y))
end

function obj:__call()
	local o=setmetatable({id=-1,x=-1,y=-1,centerx=-1,centery=-1}, obj)
    return o
end
...
function obj.scaleMoveEnlarge(center,size)
    ...
    local way1,way2
    way1=getWay(p1.x,p1.y,center.x+size.width,center.y+size.height,3,way1)
    way2=getWay(p2.x,p2.y,center.x-size.width,center.y-size.height,3,way2)
...
function obj.scaleMoveShrink(center,size)
    ...
    local way1,way2
    way1=getWay(p1.x,p1.y,center.x+0.2*size.width,center.y+0.2*size.height,3,way1)
    way2=getWay(p2.x,p2.y,center.x-0.2*size.width,center.y-0.2*size.height,3,way2)
    local step=math.min(#way1[1],#way2[1])
    local f1,f2=obj.down(nil,p1),obj.down(nil,p2)
    for i=1,step do
        f1:move(way1[1][i],way1[2][i])

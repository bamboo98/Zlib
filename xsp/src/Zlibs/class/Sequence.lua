...
function obj.del(self)
	for k,v in ipairs(self.point) do
		v:distroy()
	end
	self.point={}
	serializePoint(self)
...
function obj.add(self,p)
	...
	if type(p)~="table" then
		Zlog.fatal("参数传入错误,请传入table或string格式的颜色序列")
	end
...
if version==1 then
	...
	obj.multiFindInRect=function(self,rect,degree,hdir,vdir,priority,limit)

...
local edgeFunc={
	['自动']=function(config,data)
		...
		elseif data.screenWidth/data.screenHeight<config.minWH then
			local outHeight=data.screenHeight-data.screenWidth/config.minWH
			data.leftEdge=0
			data.rightEdge=0
		...
		local i
		i=0
		while getColor(i,data.screenHeight/5)==0 and getColor(i,data.screenHeight/5*2)==0 and getColor(i,data.screenHeight/5*3)==0 and getColor(i,data.screenHeight/5*4)==0 do
			i=i+1
		end
		local leftEdge=i
		i=0
		while getColor(data.screenWidth-1-i,data.screenHeight/5)==0 and getColor(data.screenWidth-1-i,data.screenHeight/5*2)==0 and getColor(data.screenWidth-1-i,data.screenHeight/5*3)==0 and getColor(data.screenWidth-1-i,data.screenHeight/5*4)==0 do
			i=i+1
		end
		local rightEdge=i

		i=0
		while getColor(data.screenWidth/5,i)==0 and getColor(data.screenWidth/5*2,i)==0 and getColor(data.screenWidth/5*3,i)==0 and getColor(data.screenWidth/5*4,i)==0 do
			i=i+1
		end
		local topEdge=i
		i=0
		while getColor(data.screenWidth/5,data.screenHeight-1-i)==0 and getColor(data.screenWidth/5*2,data.screenHeight-1-i)==0 and getColor(data.screenWidth/5*3,data.screenHeight-1-i)==0 and getColor(data.screenWidth/5*4,data.screenHeight-1-i)==0 do
			i=i+1
		end
		local bottomEdge=i
		keepScreen(false)
		...
		data.scale=data.developHeight/data.developHeight*data.gameHeight<data.gameWidth and data.gameHeight/data.developHeight or data.gameWidth/data.developWidth
	end,
	...
	['简单纯黑']=function(config,data)
		...
		data.leftEdge=i
		i=0
		while getColor(data.width-1-i,data.screenHeight/5)==0 and getColor(data.width-1-i,data.screenHeight/5*2)==0 and getColor(data.width-1-i,data.screenHeight/5*3)==0 and getColor(data.width-1-i,data.screenHeight/5*4)==0 do
			i=i+1
		end
		data.rightEdge=i
...
local function getColorGroup(groupname,count)
	...
	if type(groupname)=="string" then
		if type(data)~="table" then
			Zlog.fatal("获取色组传入参数错误,请传入色组名或者色组名组成的表")
		end
		local postdata={
		...
		--获取本地色组的版本号
		local versionstr="__v__"..postdata.curw..postdata.curh..postdata.curleft..postdata.curright..postdata.curtop..postdata.curbottom
		local codestr="__s__"..postdata.curw..postdata.curh..postdata.curleft..postdata.curright..postdata.curtop..postdata.curbottom
		if config.debugMode then
			versionstr="__devv__"..postdata.curw..postdata.curh..postdata.curleft..postdata.curright..postdata.curtop..postdata.curbottom
			codestr="__devs__"..postdata.curw..postdata.curh..postdata.curleft..postdata.curright..postdata.curtop..postdata.curbottom
		end
		
		postdata.version=getNumberConfig(data.gameName..versionstr..groupname,-1)
		...
		if ret=="" then--网络错误
			if count<config.tryCount then
				Zlog.warn("竹云色组数据获取失败,正在尝试获取重新获取......第%d次.",count+1)
				toast("数据获取失败,正在尝试获取重新获取......")
				return getColorGroup(groupname,count+1)
			else
				Zlog.warn("竹云色组数据获取失败,将使用本地数据")
				toast("数据获取失败,将使用本地数据")
				decoderet=getStringConfig(data.gameName..codestr..groupname,"")
				if decoderet=="" then
					Zlog.warn("脚本运行数据获取失败,且本地没有预存数据,停止运行")
					dialog("脚本运行数据获取失败,且本地没有预存数据,请检查网络连接后重试")
					lua_exit()
				end
			end
		end
		...
		if decoderet['state']=="NoUpdate" then
			...
			if ret=="" or not flag then
				...
				Zlog.warn("竹云色组数据从本地读取时出错,已清空本地数据并发起重新获取的请求",count+1)
				return getColorGroup(groupname,count+1)
			end
		else
			--把当前获取到的色组保存到本地
			...
			--更新本地版本号
			setNumberConfig(data.gameName..versionstr..groupname,decoderet['version'] or -1)
		end
		...
		for k,v in ipairs(decoderet['colors']) do
			if tonumber(v['type'])==1 then
				--单坐标处理
				rettbl[v['colorname']]=Point(v['colorname']){pos={x=tonumber(v['mainpoint']['x']),y=tonumber(v['mainpoint']['y'])}}
			elseif tonumber(v['type'])==2 then
				--双坐标/范围处理
				rettbl[v['colorname']]=Rect(v['colorname']){
					tonumber(v['area']['x1']),
					tonumber(v['area']['y1']),
					tonumber(v['area']['x2']),
					tonumber(v['area']['y2'])
	...
	elseif type(groupname)=="table" then
		...
		for _,v in ipairs(groupname) do
			...
			for k,v in pairs(tmptbl) do
				if rettbl[k] then
					dialog(string.format("特征重名!无法合并!将舍弃该特征,游戏名:%s 色组名:%s 特征名:%s",data.gameName,groupname,k))
				else
					rettbl[k]=v
				end
			end

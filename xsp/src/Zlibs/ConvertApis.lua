...
return ConstPoints[k]
end,
}

setmetatable(Point, {
	__tostring=function(self)
		return configs.Point
	end,
	__index = function(self,k)
	return ConstPoints[k]
	end,
...
function ConstRects:tl()
	return Point(self.x,self.y)
end
function ConstRects:br()
	return Point(self.x+self.width,self.y+self.height)
end
function ConstRects:size()
	return Size(self.width,self.height)
end
...
function ConstRects:intersect(r)
	...
	local r1tl,r1br=self:tl(),self:br()
	local r2tl,r2br=r:tl(),r:br()
	if math.min(r1br.x,r2br.x)-math.max(r1tl.x,r2tl.x)<=0 or math.min(r1br.y,r2br.y)-math.max(r1tl.y,r2tl.y)<=0 then
		return Rect.ZERO
	end
	return Rect(math.max(r1tl.x,r2tl.x),math.max(r1tl.y,r2tl.y),math.min(r1br.x,r2br.x)-math.max(r1tl.x,r2tl.x),math.min(r1br.y,r2br.y)-math.max(r1tl.y,r2tl.y))
end
...
__index = function(self,k)
return ConstColor3Bs[k]
end,
}

setmetatable(Color3B, {
	__tostring=function(self)
		return configs.Color3B
	end,
	__index = function(self,k)
	return ConstColor3Bs[k]
	end,
	...
	if #t==0 then
		r,g,b=0,0,0
		elseif #t==1 and type(t[1])=="number" and t[1]>=0 and t[1]<=0xffffff then
			...
			elseif #t==1 and type(t[1])=="string" then
				...
				if not c or c<0 or c>0xffffff then error("在创建Color3B对象时参数传入错误!当前传入参数为:"..table.concat(t,", ")) end
				r,g,b=math.floor(c/0x10000),math.floor(c/0x100)%0x100,c%0x100
...
local Color3FMeta={
__tostring=function(self)
return string.format("0x%2x%2x%2x",round(self.r*0xff),round(self.g*0xff),round(self.b*0xff))
end,
__index = function(self,k)
return ConstColor3Fs[k]
end,
}

setmetatable(Color3F, {
	__tostring=function(self)
		return configs.Color3F
	end,
	__index = function(self,k)
	return ConstColor3Fs[k]
	end,
	...
	if #t==0 then
		r,g,b=0,0,0
		elseif #t==1 and type(t[1])=="number" and t[1]>=0 and t[1]<=0xffffff then
			r,g,b=math.floor(t[1]/0x10000)/0xff,math.floor(t[1]/0x100)%0x100/0xff,t[1]%0x100/0xff
			elseif #t==1 and type(t[1])=="string" then
				local c=tonumber(t[1],16)
				if not c or c<0 or c>0xffffff then error("在创建Color3B对象时参数传入错误!当前传入参数为:"..table.concat(t,", ")) end
				r,g,b=math.floor(c/0x10000)/0xff,math.floor(c/0x100)%0x100/0xff,c%0x100/0xff
				elseif #t==3 and type(t[1])=="number" and type(t[2])=="number" and type(t[3])=="number" then
					r,g,b=...
...
function ConstColor3Fs:toInt()
	return round(self.r*0xff)*0x10000+round(self.g*0xff)*0x100+round(self.b*0xff)
end
...
Constxmods.PLATFORM = getOSType()=="android" and xmod.PLATFORM_ANDROID or xmod.PLATFORM_IOS
...
function Constxmods.setConfig(key,value)
	if key==xmod.EXPECTED_ORIENTATION then
		...
		elseif key==xmod.SCREENCAP_POLICY then
			setSysConfig("screencap_policy", value==xmod.SCREENCAP_POLICY_AGGRESSIVE and "aggresive" or "default")
...
local UserInfoMeta={
__tostring=function(self)
return "UserInfo模块,用户id:"..self.id..",付费方式:"..self.membership..",剩余时间:"..self.expiredTime
end,
__index = function(self,k)
return ConstUserInfos[k]
end,
}
...
setmetatable(UserInfo, {
	...
	__call = function(self)
		...
		return setmetatable({id=id,membership=buyState,expiredTime=validTime}, UserInfoMeta),res
	end
})
...
setmetatable(script, {
	__tostring=function(self)
		return configs.script
	end,
	__index = function(self,k)
	return Constscripts[k]
	end,
	__newindex = function(self,k,v)
...
function Constscreens.setMockMode(mode)
	if type(mode)~="number" then error("setMockMode函数传入参数错误") end
...
function Constscreens.getRGB(...)
	...
	elseif #t==2 and type(t[1])=="number" and type(t[2])=="number" then
		if screen.Transform and (screen.MockMode==screen.MOCK_INPUT or screen.MockMode==screen.MOCK_BOTH) then
			TrRect=screen.Transform(screen.MOCK_INPUT_FIXED,Rect(t[1],t[2],0,0))
		else
...
local function Compare_Color(c1,c2,Fuzz,Offest)--颜色比较
	if Offest then
		return math.abs(c1.r-c2.r)<=Offest.r and math.abs(c1.g-c2.g)<=Offest.g and math.abs(c1.b-c2.b)<=Offest.b
	end
	return math.abs(c1.r-c2.r)<=0xff-0xff*Fuzz/100 and math.abs(c1.g-c2.g)<=0xff-0xff*Fuzz/100 and math.abs(c1.b-c2.b)<=0xff-0xff*Fuzz/100
end
...
function Constscreens.matchColors(...)
	...
	if type(t1)=="table" then
		...
		for i=1,#t1 do
			...
			if not Compare_Color(getc,Color3B(t1[i].color),t1[i].fuzz or Fuzz,t1[i].offset) then
				return false
			end
		end
	else
...
function Constscreens.findImage(rect,image,fuzzness,priority,ignoreColor)
	...
	if type(image)~="string" then
		error("findImage函数因1.9引擎限制,只支持res文件夹中的图像,请传入字符串")
	end
	fuzzness=tonumber(fuzzness) or 100
	priority=priority or screen.PRIORITY_DEFAULT
	local x,y=findImageInRegionFuzzy(image,fuzzness,rect.x,rect.y,rect.x+rect.width,rect.y+rect.height,ignoreColor)
...
local PRIORITY_LIST={
	...
	[bit.bor(bit.bor(screen.PRIORITY_LEFT_FIRST,screen.PRIORITY_UP_FIRST),screen.PRIORITY_HORIZONTAL_FIRST)]={0,0,0},
	[bit.bor(bit.bor(screen.PRIORITY_RIGHT_FIRST,screen.PRIORITY_UP_FIRST),screen.PRIORITY_HORIZONTAL_FIRST)]={1,0,0},
	[bit.bor(bit.bor(screen.PRIORITY_LEFT_FIRST,screen.PRIORITY_DOWN_FIRST),screen.PRIORITY_HORIZONTAL_FIRST)]={0,1,0},
	[bit.bor(bit.bor(screen.PRIORITY_LEFT_FIRST,screen.PRIORITY_UP_FIRST),screen.PRIORITY_VERTICAL_FIRST)]={0,0,1},
	[bit.bor(bit.bor(screen.PRIORITY_RIGHT_FIRST,screen.PRIORITY_DOWN_FIRST),screen.PRIORITY_HORIZONTAL_FIRST)]={1,1,0},
	[bit.bor(bit.bor(screen.PRIORITY_LEFT_FIRST,screen.PRIORITY_DOWN_FIRST),screen.PRIORITY_VERTICAL_FIRST)]={0,1,1},
	[bit.bor(bit.bor(screen.PRIORITY_RIGHT_FIRST,screen.PRIORITY_UP_FIRST),screen.PRIORITY_VERTICAL_FIRST)]={1,0,1},
	[bit.bor(bit.bor(screen.PRIORITY_RIGHT_FIRST,screen.PRIORITY_DOWN_FIRST),screen.PRIORITY_VERTICAL_FIRST)]={1,1,1},
}
...
function Constscreens.findColor(rect,color,globalFuzz,priority)
	...
	priority=priority and PRIORITY_LIST[priority] or PRIORITY_LIST[screen.PRIORITY_DEFAULT]
	local t1,TrRect
	...
	for i=1,#t1 do
		--坐标转换
		if screen.Transform and (screen.MockMode==screen.MOCK_INPUT or screen.MockMode==screen.MOCK_BOTH) then
			TrRect=screen.Transform(screen.MOCK_INPUT_FIXED,Rect(t1[i].pos.x,t1[i].pos.y,0,0))
		else
			TrRect=Rect(t1[i].pos.x,t1[i].pos.y,0,0)
		end
		t1[i]={x=TrRect.x,y=TrRect.y,color=t1[i].color,degree=t1[i].fuzz or globalFuzz,offset=t1[i].offset}
	end
	--范围换算
	if screen.Transform and (screen.MockMode==screen.MOCK_INPUT or screen.MockMode==screen.MOCK_BOTH) then
	...
	--调用找色
	local x,y=findColor({TrRect.x,TrRect.y,TrRect.x+TrRect.width,TrRect.y+TrRect.height},t1,globalFuzz,unpack(priority))
	if x==-1 then
		return Point.INVALID
	end
	TrRect=Rect(x,y,0,0)
	--返回值换算
	if screen.Transform and (screen.MockMode==screen.MOCK_OUTPUT or screen.MockMode==screen.MOCK_BOTH) then
...
function Constscreens.findColors(rect,color,globalFuzz,priority,limit)
	...
	--调用找色
	local tbl=RepairFindColors({TrRect.x,TrRect.y,TrRect.x+TrRect.width,TrRect.y+TrRect.height},t1,globalFuzz,hdir,vdir,priority,limit)
...
setmetatable(touch, {
	__tostring=function(self)
		return configs.touch
	end,
	__index = function(self,k)
		return Consttouchs[k]
		end,
	__newindex = function(self,k,v)
...
function Consttouchs.press(type,longPress)
	pressKey(KEY_LIST[type],longPress)
end
...
function Conststorages.undo()
	storageData={}
end

function Conststorages.purge()
	storageData={}
	print("警告:1.9引擎不支持清除保存的数据!该操作和storage.undo相同!(来自storage.purge函数)")
end
...
function ConstUIs.toast(msg,lenth)
	toast(msg)
end
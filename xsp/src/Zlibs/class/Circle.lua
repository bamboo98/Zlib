...
function obj:__newindex(k,v)
	rawset(self,k,v)
	Zlog.warn("不建议的操作:设置了[%s]对象中[%s]元素的值为[%s]",self._tag,tostring(k),tostring(v))
end
...
function obj:__tostring()
	return string.format("%s(%s ,%d)",obj.__tag,tostring(Point(self.x,self.y)),self.r)
end
function obj:__add(r)
	return obj:__call(self.x,self.y,self.r+r)
end
function obj:__sub(r)
	return obj:__call(self.x,self.y,self.r-r)
end
function obj:__mul(n)
	return obj:__call(self.x,self.y,self.r*n)
end
...
--相等
function obj:__eq(s)
	return self.r==s.r and self.x==s.x and self.y==s.y
end
function obj:__call(...)
	local o={}
	local t={...}
